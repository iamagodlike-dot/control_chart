...
export default function Warehouse({ onOpenJob }) {
  ...
  return (
    <div style={{ background: '#fafaf8', fontFamily: "'IBM Plex Sans','Helvetica Neue',sans-serif", color: '#1a1a1a', borderRadius: 12, overflow: 'hidden' }}>
      <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600&family=IBM+Plex+Mono:wght@400;500&display=swap" rel="stylesheet" />

      <div style={{ background: '#0f0f0f', color: '#fff', padding: '0 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', height: 52 }}>
        <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontWeight: 500, fontSize: 13, letterSpacing: 2, textTransform: 'uppercase' }}>СКЛАД ЗАПЧАСТЕЙ</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e8e8e8', display: 'flex', padding: '0 1.5rem' }}>
...
export function CellPickerModal({ currentCellIds = [], onSave, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1050, padding: '1rem' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 720, maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontWeight: 600 }}>Выбор ячеек склада</span>
          <span style={{ color: '#999', fontSize: 12 }}>можно выбрать несколько</span>
          <div style={{ flex: 1 }} />
              ...
              {config.zones.map((zone) => (
                <div key={zone.id}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <div style={{ background: '#0f0f0f', color: '#fff', fontFamily: "'IBM Plex Mono',monospace", padding: '3px 10px', borderRadius: 4, fontSize: 12 }}>{zone.id}</div>
                    <span style={{ fontWeight: 600, fontSize: 14 }}>{zone.label}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: `repeat(${zone.cols}, 1fr)`, gap: 6 }}>
                      ...
                      Array.from({ length: zone.cols }, (_, c) => {
                        ...
                        return (
                          <button
                            ...
                            style={{
                              border: `2px solid ${isSelected ? '#1976d2' : isOccupied ? '#ff9800' : '#4caf50'}`,
                              background: isSelected ? '#e3f2fd' : isOccupied ? '#fff8e1' : '#e8f5e9',
                              borderRadius: 6, padding: '8px 4px', cursor: isOccupied ? 'not-allowed' : 'pointer',
                              fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, fontWeight: 600,
                              color: isSelected ? '#1976d2' : isOccupied ? '#e65100' : '#2e7d32',
        ...
        <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid #eee', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 13, color: '#666', flex: 1 }}>
            {selected.length ? `Выбрано: ${selected.join(', ')}` : 'Ничего не выбрано'}
          </span>
          <button onClick={onClose} style={{ background: 'none', border: '1px solid #ddd', borderRadius: 6, padding: '7px 16px', cursor: 'pointer', fontSize: 13 }}>Отмена</button>
          <button onClick={() => { onSave(selected); onClose(); }} style={{ background: '#0f0f0f', color: '#fff', border: 'none', borderRadius: 6, padding: '7px 18px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Сохранить</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 640, maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          {isLinked && (
            <div style={{ background: '#e3f2fd', border: '1px solid #90caf9', borderRadius: 6, padding: '8px 12px', fontSize: 12, color: '#1565c0', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {onOpenJob && (
                <button
                  ...
                  style={{ background: '#1565c0', color: '#fff', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: 'pointer', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}
                >
                  Открыть заказ-наряд →
                </button>
              )}
          ...
          {!isOccupied && onLinkJob && (
            <div style={{ marginTop: 20, borderTop: '1px solid #eee', paddingTop: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, color: '#333', marginBottom: 8 }}>ПРИВЯЗАТЬ К СУЩЕСТВУЮЩЕМУ ЗАКАЗУ</div>
              <input value={linkQuery} onChange={(e) => setLinkQuery(e.target.value)} placeholder="Поиск по авто, гос. номеру, № заказа..." style={inputSt} />
              <div style={{ maxHeight: 160, overflow: 'auto', marginTop: 8, border: linkMatches.length ? '1px solid #eee' : 'none', borderRadius: 6 }}>
                {jobOptions === null && <div style={{ color: '#999', fontSize: 12, padding: '8px 4px' }}>Загрузка заказов...</div>}
                {jobOptions !== null && linkMatches.length === 0 && (
                  <div style={{ color: '#999', fontSize: 12, padding: '8px 4px' }}>Ничего не найдено</div>
                )}
                {linkMatches.map((j) => (
                  <div
                    key={j.id}
                    onClick={() => onLinkJob(id, j)}
                    style={{ padding: '8px 10px', cursor: 'pointer', fontSize: 13, display: 'flex', justifyContent: 'space-between', gap: 10, borderBottom: '1px solid #f2f2f2' }}
                  >
                    <span>{j.car_model}{j.plate_number ? ` (${j.plate_number})` : ''}</span>
                    <span style={{ color: '#999' }}>{j.order_number || '—'}</span>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1010, padding: '1rem' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '80vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontWeight: 600, fontSize: 15 }}>{id}</div>
          <span style={{ color: '#666', fontSize: 13 }}>История движения</span>
          <div style={{ flex: 1 }} />
          {isOccupied && <button onClick={() => { if (window.confirm('Освободить ячейку?')) onFree(id); }} style={{ background: '#fff3e0', border: '1px solid #ffcc02', color: '#e65100', borderRadius: 6, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Освободить</button>}
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#999' }}>×</button>
        </div>

...
export const ROLES = {
  ...
  // Мастер-приёмщик: зовёт машины на дефектовку и встречает их на заезде. Его
  // экран — «Дефектовка» (кого пригласить, кто когда приедет, напоминания). Плюс
  // весь цех и запчасти: он на телефоне у клиента и должен отвечать «что дальше»,
  // «когда встанет в работу» и «где деталь», не бегая к управленцу.
  //
  // «Заявки» и «Закупки» роли НЕ положены: там записывают потраченные деньги, а
  // денежные экраны приёмщику закрыты (решение владельца).
  receptionist: {
    label: 'Мастер-приёмщик',
    hint: 'Дефектовка, цех и запчасти; денег и настроек не видит',
    tabs: ['intake', 'approval', 'gantt', 'board', 'monitor',
